  ...
  <view class="animate-fade-in-up relative z-1 px-[24rpx] py-[16rpx] pb-[24rpx]">
    ...
    <view class="grid grid-cols-3 gap-[16rpx]">
      ...
      >
        ...
        >
          <wd-icon v-if="entry.icon && !entry.icon.includes(':')" :name="entry.icon" size="48rpx" :color="entry.highlight ? 'var(--wot-color-theme)' : 'var(--wot-color-warning)'" />
          <view v-else-if="entry.icon" class="text-[40rpx] leading-[40rpx] flex items-center justify-center" :class="[`i-${entry.icon}`, entry.highlight ? 'text-[var(--wot-color-theme)]' : 'text-[var(--wot-color-warning)]']" />
        </view>
