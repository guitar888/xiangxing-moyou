...
<template>
  <view class="min-h-screen bg-base pb-[40rpx]">
    ...
    <view class="px-[24rpx] pt-[20rpx]">
      <view class="flex items-center gap-[16rpx] border border-warning/30 rounded-[16rpx] bg-warning/10 p-[20rpx]">
        <text class="i-carbon:warning flex-shrink-0 text-[40rpx] text-warning" />
        <text class="flex-1 text-[24rpx] text-warning">
          出行前请提前了解当日限行政策，遵守交通规则
        </text>
      </view>
    </view>
    ...
    <view class="mt-[24rpx] px-[24rpx]">
      <text class="mb-[16rpx] block text-[28rpx] text-white font-600">
        限行政策
      </text>
      <view class="space-y-[16rpx]">
        <view
          ...
          :key="policy.id"
          class="rounded-[16rpx] bg-card p-[24rpx]"
          @click="showPolicyDetail(policy)"
        >
          <view class="flex items-start justify-between">
            <view class="flex-1">
              <text class="mb-[8rpx] block text-[26rpx] text-white font-600">
                {{ policy.title }}
              </text>
              <text class="line-clamp-2 text-[22rpx] text-gray">
                {{ policy.content }}
              </text>
            </view>
            <text class="i-carbon:chevron-right ml-[16rpx] text-[24rpx] text-gray" />
          </view>
          <view v-if="policy.area || policy.time" class="mt-[16rpx] flex gap-[24rpx]">
    ...
    <!-- 合规声明 -->
    <view class="mt-[40rpx] px-[24rpx] text-center">
      <text class="text-[20rpx] text-gray">
        本平台仅提供限行信息参考，具体政策以官方公告为准
      </text>
    </view>
    ...
    <view
      v-if="activePolicy"
      class="fixed left-1/2 top-1/2 z-[999] w-[650rpx] rounded-[24rpx] bg-card p-[32rpx] -translate-x-1/2 -translate-y-1/2"
    >
      <text class="mb-[16rpx] block text-[30rpx] text-white font-600">
        {{ activePolicy.title }}
      </text>
      <text class="mb-[20rpx] block text-[24rpx] text-gray leading-[36rpx]">
        {{ activePolicy.content }}
      </text>
      <view v-if="activePolicy.area" class="mb-[12rpx] flex items-center gap-[8rpx]">
        <text class="i-carbon:location text-[24rpx] text-primary" />
        <text class="text-[24rpx] text-white">
          {{ activePolicy.area }}
        </text>
      </view>
      <view v-if="activePolicy.time" class="mb-[12rpx] flex items-center gap-[8rpx]">
      ...
      <button
        class="mt-[24rpx] w-full rounded-[12rpx] bg-primary py-[20rpx] text-[26rpx] text-white font-500"
        @click="closePolicyDetail"
      >
        我已知晓
      </button>
