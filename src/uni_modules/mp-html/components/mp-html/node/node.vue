...
<template>
  <view :id="attrs.id" :class="`_block _${name} ${attrs.class}`" :style="attrs.style">
    <block v-for="(n, i) in childs" :key="i">
      ...
      <!-- #endif -->
      <!-- #ifdef MP-KUAISHOU -->
      <image v-else-if="n.name === 'img'" :id="n.attrs.id" :class="`_img ${n.attrs.class}`" :style="(ctrl[i] === -1 ? 'display:none;' : '') + n.attrs.style" :src="n.attrs.src" :lazy-load="opts[0]" :data-i="i" @load="imgLoad" @error="mediaError" @tap.stop="imgTap" />
      <!-- #endif -->
      <!-- #ifdef APP-PLUS && VUE3 -->
      ...
      <!-- 视频 -->
      <!-- #ifdef APP-PLUS -->
      <view v-else-if="n.html" :id="n.attrs.id" :class="`_video ${n.attrs.class}`" :style="n.attrs.style" :data-i="i" @vplay.stop="play" v-html="n.html" />
      <!-- #endif -->
      <!-- #ifndef APP-PLUS -->
      ...
      <view v-else-if="n.c === 2" :id="n.attrs.id" :class="`_block _${n.name} ${n.attrs.class}`" :style="`${n.f};${n.attrs.style}`">
        <node v-for="(n2, j) in n.children" :key="j" :style="n2.f" :name="n2.name" :attrs="n2.attrs" :childs="n2.children" :opts="opts" />
      </view>
